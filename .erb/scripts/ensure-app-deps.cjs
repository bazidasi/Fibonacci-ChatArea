...
exports.default = async function ensureAppDeps(context) {
  ...
  // --workspaces=false is REQUIRED: release/app is a member of the root bun
  // workspaces ("workspaces": ["packages/*", "release/app"]), and npm walks up
  // from cwd to detect that root. Without this flag npm switches to the root
  // prefix and fails with EUSAGE ("npm ci can only install with an existing
  // package-lock.json") because the repo's lockfile is bun.lock, not a root
  // package-lock.json. The env vars guard against npm_config_workspaces being
  // inherited from a parent process (e.g. electron-builder workspace detection).
  execSync('npm ci --omit=dev --ignore-scripts --workspaces=false --include-workspace-root=false', {
    cwd: appDir,
    stdio: 'inherit',
    env: {
      ...process.env,
