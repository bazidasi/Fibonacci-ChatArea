...
当工具因为缺少配置而失败（例如选择了 Tavily 但没有填写 API Key、或选择了 Fibonacci AI 但没有 license），消息中的工具气泡会自动展开，显示本地化的错误说明，并提供一键跳转链接（前往设置页输入 API Key、打开 license 页面、升级账户等），不再只是"工具调用失败"的红色提示。
