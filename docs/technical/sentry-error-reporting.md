...
生成链路统一使用 `src/shared/models/error-classification.ts` 判断预期错误，覆盖 Fibonacci AI 额度/License/能力错误、Provider API 错误、网络错误和已有 fallback 的 OCR/图片能力错误。不要在各入口维护不同的错误白名单。
