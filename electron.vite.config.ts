  ...
  const config: ElectronViteConfig = {
    main: {
      plugins: [
        ...(isProduction
          ? [
              ...(process.env.CHATBOX_BUNDLE_STATS === 'true'
                ? [
                    visualizer({
          ...
          output: {
            ...
            manualChunks(id) {
              ...
              if (normalizedId.includes('/node_modules/')) {
                // Split large vendor chunks (kept disjoint to avoid circular chunk merging,
                // which inflates peak memory during rollup's chunk render phase)
                if (isNodeModulePackage('@ai-sdk') || isNodeModulePackage('ai') || isNodeModulePackage('@mantine') || isNodeModulePackage('@tabler')) {
                  return 'vendor-ui-ai'
                }
