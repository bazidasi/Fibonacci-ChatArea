...
describe('trackGenerateEvent', () => {
  ...
  test.each(['file_preprocess_failed', 'file_storage_quota_exceeded'] as const)(
    ...
    (codeName) => {
      ...
      if (!error) throw new Error(`Expected a known Fibonacci AI error: ${codeName}`)
      const message = {
        id: 'message-1',
        role: 'assistant',
        contentParts: [],
  ...
  test('persists Fibonacci AI OCR quota exhaustion separately from main-model quota exhaustion', () => {
    const cause = ChatboxAIAPIError.fromCodeName('quota', 'token_quota_exhausted')
    expect(cause).not.toBeNull()
    if (!cause) throw new Error('Expected a known Fibonacci AI quota error')
    const error = new OCRError('Fibonacci AI', cause)
    const message = {
      id: 'message-1',
      role: 'assistant',
      contentParts: [],
  ...
  test('preserves the daily-free scope when Fibonacci AI OCR exhausts quota', () => {
    const cause = ChatboxAIAPIError.fromCodeName('daily quota', 'free_token_quota_exhausted')
    expect(cause).not.toBeNull()
    if (!cause) throw new Error('Expected a known Fibonacci AI free quota error')
    const error = new OCRError('Fibonacci AI', cause)
    const message = {
      id: 'message-1',
      role: 'assistant',
