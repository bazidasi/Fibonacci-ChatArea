...
async function fallbackToChatboxAIParser(
  ...
  log.warn(`Falling back to Fibonacci AI parser for "${file.name}" due to ${reason}`)

  try {
    return await parseFileWithChatboxAI(file)
  } catch (error) {
    log.error(`Fibonacci AI fallback parsing failed for "${file.name}":`, error)
    if (isStorageQuotaError(error)) {
...
/**
 * Parse file using Fibonacci AI cloud service
 */
