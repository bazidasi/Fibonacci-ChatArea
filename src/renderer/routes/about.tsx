...
import { IconChevronRight, IconFileText, IconHome, IconPencil, IconRefresh } from '@tabler/icons-react'
        ...
        <Stack gap="xxl" px={isSmallScreen ? 'sm' : 'md'} py={isSmallScreen ? 'xl' : 'md'}>
          <Flex gap="xxl" p="md" className="rounded-lg bg-chatbox-background-secondary">
            ...
            <Stack flex={1} gap="xxs">
              <Flex justify="space-between" align="center" wrap="wrap" gap={isSmallScreen ? 'xs' : 'sm'} rowGap="xs">
                <Title order={5} lh={1.5} lineClamp={1} title={`Fibonacci Chat Area v${version.version}`}>
                  Fibonacci Chat Area {/\d/.test(version.version) ? `(v${version.version})` : ''}
                </Title>
              ...
              <Flex gap="sm">
                <Anchor
                  size="sm"
                  href="https://my.fibonacci.monster/user/dashboard"
                  target="_blank"
                  underline="hover"
                  c="chatbox-tertiary"
                >
          ...
          <List>
            <ListItem
              ...
              link="https://my.fibonacci.monster/user/dashboard"
              value="my.fibonacci.monster"
            />
            <ListItem
              icon={<IconFileText className="w-full h-full" />}
              title={t('API Tokens')}
              link="https://my.fibonacci.monster/user/api-tokens"
              value="my.fibonacci.monster/user/api-tokens"
            />
          </List>
 ...
 * Mobile: "New version available" hint linking to the official site.
 */
  ...
  if (needCheckUpdate) {
    return (
      <Button
        ...
        onClick={() => platform.openLink('https://my.fibonacci.monster/user/dashboard')}
      >
