...
export function RouteComponent() {
  ...
  const providers = useMemo<ProviderInfo[]>(() => {
    const systemProviders = SystemProviders().filter(
      (p) =>
        p.id !== 'chatbox-ai' && // Fibonacci fork: legacy Fibonacci AI provider hidden
        !(isExceeded && p.name.toLocaleLowerCase().match(/openai|claude|gemini/i))
    )
    // Put Fibonacci AI first
    const fibonacci = systemProviders.find((p) => p.id === 'fibonacci')
    const others = systemProviders.filter((p) => p.id !== 'fibonacci')
    return [...(fibonacci ? [fibonacci] : []), ...others, ...(customProviders || [])].map((p) => ({
      ...p,
      ...(providersMap?.[p.id] || {}),
    }))
