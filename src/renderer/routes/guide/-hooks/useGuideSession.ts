  ...
  useEffect(() => {
    ...
    if (enterCompleted) {
      ...
      const configCompleteMsg = t(
        "You've already completed the setup and can use Fibonacci Chat Area normally.\n\nIf you have any questions about Fibonacci AI, feel free to ask me here."
      )
      setMessages([
        {
          id: generateMessageId(),
      ...
      const guideUrl = 'https://my.fibonacci.monster/user/dashboard'
      const helpCenterUrl = 'https://my.fibonacci.monster/user/api-tokens'

      const greeting = isChinese
        ? [
            t(`## 👋 Hey! I'm Boxy, your setup guide assistant.

Fibonacci Chat Area is an **all-in-one AI chat client** powered by Fibonacci AI, supporting models like fibonacci-1-pro-max, fibonacci-1-agentic, fibonacci-2-coder, and more.

### ✨ Key Features
- 🔐 **Local First** — Your data stays on your device, ensuring privacy and security
- 🎯 **Multi-Model Support** — One app, chat with all Fibonacci AI models
...
- 🆘 [API Tokens](`),
            helpCenterUrl,
            t(`) — Create and manage your API tokens
- 📮 Contact us: support@fibonacci.monster
...
**Now, let me help you get set up!** First, tell me about your AI experience:`),
  ...
  const selectUserType = useCallback(
    async (type: UserType) => {
      ...
      const responseContent =
        type === 'novice'
          ? t(
              'Great! Fibonacci AI is our all-in-one service designed for new users - powerful models with no complex setup required.\n\nFirst, get your API token at [my.fibonacci.monster/user/api-tokens](https://my.fibonacci.monster/user/api-tokens), then click the button below to paste it into the Fibonacci AI provider settings.'
            )
          : t(
  ...
  const renderCelebration = useCallback(
    async (options?: { waitForWindowFocusBeforeAutoNavigate?: boolean }) => {
      ...
      await streamFixedMessage(t("Awesome, everything is ready! Let's start your first Fibonacci AI chat."), [
  ...
  const markGuideCompleted = useCallback(async () => {
    ...
    if (hasLicense) {
      ...
      await streamFixedMessage(
        t(
          "You're logged in! Claim your **free plan** below to unlock Fibonacci AI features. If you have any questions, feel free to click the Help button in the bottom left corner anytime."
        ),
        [
          {
  ...
  const onClaimStart = useCallback(async () => {
    ...
    await streamFixedMessage(t("We're waiting for you to finish on my.fibonacci.monster..."), [
      {
  ...
  const debugSkipToLoginSuccess = useCallback(() => {
    ...
    const successText = t("Awesome, everything is ready! Let's start your first Fibonacci AI chat.")
  ...
  const debugTriggerRoundLimit = useCallback(() => {
    ...
    fakeMessages.push({
      id: generateMessageId(),
      role: 'assistant',
      content: t('Welcome to Fibonacci Chat Area!'),
      parts: [{ type: 'text', text: t('Welcome to Fibonacci Chat Area!') }],
    })
