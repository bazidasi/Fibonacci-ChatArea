  ...
  const renderErrorTips = () => {
    ...
    if (isSessionAttachmentRagAuthError(errorCode)) {
      return (
        <Text>
          {t(
            'This large file needs to finish indexing. Sign in to , then retry this file. If you do not want to use , remove the file and upload a smaller attachment instead.'
          )}
        </Text>
      )
    }
