...
describe('FileParseError', () => {
  ...
  test('lets desktop users switch parsers when Fibonacci AI parsing has no account license', async () => {
    ...
    const openSettings = screen.getByText('Sign in to Fibonacci AI')
  ...
  test.each(['web', 'mobile'])(
    ...
    async (type) => {
      ...
      expect(await screen.findByText('File: lecture.pdf')).toBeTruthy()
      expect(screen.getByText('Sign in to Fibonacci AI').tagName).toBe('BUTTON')
