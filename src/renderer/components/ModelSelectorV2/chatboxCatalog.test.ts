...
function createCatalog(plan: string): ChatboxAIModelList {
  return {
    provider: { id: 'chatbox-ai', name: 'Fibonacci AI' },
    license: { plan },
    groups: [
      {
        id: 'advanced',
...
describe('chatboxCatalog', () => {
  ...
  test('matches search against provider, model id, and model name', () => {
    ...
    expect(modelMatchesSearch(model, 'chatbox', 'Fibonacci AI')).toBe(true)
