...
import { attachLenisScroller } from '@/lib/animations/neo-animations'
...
const MessageList = forwardRef<MessageListRef, MessageListProps>((props, ref) => {
  ...
  // Lenis smooth scrolling lives on the virtualized scroller element.
  const lenisCleanupRef = useRef<(() => void) | null>(null)
  // keep a ref in sync so the stable scroller callback can read the latest platform type
  const platformTypeRef = useRef<string | undefined>(undefined)
  const handleVirtuosoScrollerRef = useCallback((ref: HTMLElement | Window | null) => {
    ...
    if (ref && ref instanceof HTMLElement && platformTypeRef.current !== 'mobile') {
      lenisCleanupRef.current = attachLenisScroller(ref)
    }
  }, [])
  useEffect(() => {
    return () => {
