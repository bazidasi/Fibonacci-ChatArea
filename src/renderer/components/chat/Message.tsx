...
import { SystemAvatar, UserAvatar } from '../common/Avatar'
  ...
  const messageContent = (
    <>
      <div
        ...
        className={cn(
          msg.role === 'assistant' ? 'w-full min-w-0' : 'inline-block max-w-full min-w-0',
          isBubbleLayout
            ? msg.role === 'user'
  ...
  if (isClassicMessage) {
    return (
      ...
      >
        ...
        >
          {shouldShowAvatar && (
            <Box className="relative shrink-0 mt-1">
              {msg.role === 'user' ? (
                <UserAvatar avatarKey={userAvatarKey} onClick={() => navigateToSettings('/chat')} />
              ) : (
                <SystemAvatar sessionType={props.sessionType} onClick={onClickAssistantAvatar} />
              )}
            </Box>
          )}
