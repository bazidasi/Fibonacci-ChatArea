...
export function QuotaExhaustedCard({ kind, action, onAction, onConfigureOcr }: QuotaExhaustedCardProps) {
  ...
  if (kind === 'ocr-quota-exhausted') {
    description = shouldBuyExpansionPack
      ? t(
          'The current model uses OCR to process images, and its quota for the current period is used up. Buy an expansion pack or change the default OCR model to continue.'
        )
      : t(
          'The current model uses OCR to process images, and its quota for the current period is used up. Upgrade your plan or change the default OCR model to continue.'
        )
  } else if (kind === 'free-ocr-quota-exhausted') {
    description = t(
      "The current model uses OCR to process images, and today's free OCR points are used up. Free points reset daily; upgrade your plan or change the default OCR model to continue."
    )
  } else if (isFreeQuota) {
        ...
        <Box style={{ flex: 1, minWidth: 0 }}>
          <Stack gap={4}>
            <Text size="sm" fw={600} lh={1.45}>
              {isOcrQuota ? t(' OCR points are used up') : t('Your points are used up')}
            </Text>
