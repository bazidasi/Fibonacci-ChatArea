    ...
    >
      <Flex align="center" gap={6} className="min-w-0 flex-1">
        ...
        <Text size="xs" c="chatbox-secondary" truncate>
          {isSmallScreen
            ? t('Web Search was skipped: sign in to')
            : t(
                'Web Search was skipped because requires sign-in. Future searches in this chat will fail until you sign in or change providers.'
              )}
        </Text>
      </Flex>
      <Flex align="center" gap={4} className="shrink-0">
        <Button h={24} px={10} radius={12} size="compact-xs" variant="light" onClick={() => navigateToSettings()}>
          {isSmallScreen ? t('Sign in') : t('Sign in to')}
        </Button>
        {!isSmallScreen && (
          <Button
            h={24}
