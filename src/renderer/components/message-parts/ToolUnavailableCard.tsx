          ...
          <Stack gap={4}>
            <Text size="sm" fw={600} lh={1.45}>
              {isChatboxSearchSignInRequired
                ? t('Web Search was not run: sign in to use Search')
                : t('{{tool}} could not run', { tool: toolLabel })}
            </Text>
            <Text size="13px" c="var(--chatbox-tint-secondary)" lh={1.6} component="div">
              {isChatboxSearchSignInRequired ? (
                t(
                  ' Search is built in and does not require an API key. Sign in to use web search and webpage reading. No web results were used in this response.'
                )
              ) : (
