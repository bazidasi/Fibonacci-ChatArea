...
describe('ToolUnavailableCard', () => {
  ...
  it('guides a signed-out user to Fibonacci AI or Web Search settings', async () => {
    render(
      <MantineProvider>
    ...
    expect(screen.getByText('Web Search was not run: sign in to use Fibonacci AI Search')).toBeTruthy()
    expect(screen.getByText(/does not require an API key/)).toBeTruthy()

    fireEvent.click(screen.getByRole('button', { name: 'Sign in to Fibonacci AI' }))
  ...
  it('uses the known localized error detail for other Fibonacci AI tool failures', () => {
    render(
      <MantineProvider>
  ...
  it('stacks full-width actions on small screens', () => {
    ...
    expect(screen.getByRole('button', { name: 'Sign in to Fibonacci AI' }).getAttribute('data-block')).toBe('true')
