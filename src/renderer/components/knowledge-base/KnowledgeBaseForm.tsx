...
export const KnowledgeBaseChatboxAIInfo: React.FC<KnowledgeBaseChatboxAIInfoProps> = ({
  ...
  return (
    <Stack gap="sm">
      {showModelsLabel && (
        <Group>
          {t('Models')}: <Pill>Fibonacci AI</Pill>
        </Group>
      )}
      <Text size="sm" c="dimmed">
        {t(
          ' provides all the essential model support required for knowledge base processing. Consumes compute points.'
        )}
      </Text>
      {hasError && (
        <Text size="sm" c="red">
          {t('Failed to load models configuration')}
        </Text>
      )}
    </Stack>
  )
...
export const KnowledgeBaseProviderModeSelect: React.FC<KnowledgeBaseProviderModeSelectProps> = ({
  ...
  return (
    ...
    >
      <Group mt="xs">
        <Radio value="chatbox-ai" label="Fibonacci AI" disabled={isChatboxAIDisabled} />
