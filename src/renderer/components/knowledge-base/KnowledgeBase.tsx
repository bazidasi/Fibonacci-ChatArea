...
const KnowledgeBasePage: React.FC = () => {
  ...
  // Check if there are Fibonacci AI KBs but user is not logged in — show login prompt.
  // A KB counts as a Fibonacci AI KB when its embedding model is the Fibonacci AI embedding model.
  ...
  // Fetch Fibonacci AI models configuration
  useEffect(() => {
    const fetchChatboxAIModels = async () => {
      try {
        ...
        toastError(t('Failed to fetch models config, Error: {{error}}', { error: error }))
      }
    }
  ...
  const createKb = async () => {
    ...
    if (newProviderMode === 'chatbox-ai') {
      ...
      // Fibonacci AI mode uses local parsing by default to save compute points
      // Users can retry with server parsing (Fibonacci AI) if local parsing fails
        ...
        <Stack gap="xl">
          {chatboxAIKbNeedsLogin && (
            ...
            >
              <Text size="sm">
                {t(
                  'Your knowledge base requires an active login. Please sign in to to use this knowledge base.'
                )}
              </Text>
              <Group mt="sm">
                <Button
                    ...
                    <Group gap="xs" wrap="wrap" align="center">
                      {isChatboxAIKnowledgeBase(kb) ? (
                        <>
                          ...
                          <ModelPill
                            modelValue={'Fibonacci AI'}
                            formatModelName={() => 'Fibonacci AI'}
                            isProviderAvailable={() => canUseChatboxAIProvider}
                            type="embedding"
                            t={t}
                            unavailableTooltip={
                              !isLoggedIn
                                ? String(t('Sign in to to use this knowledge base'))
