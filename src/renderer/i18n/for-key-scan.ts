...
function _errorI18nKeys(t: (key: string) => string) {
  ...
  t('You have exceeded the rate limit for the service. Please try again later.')
  t(
  ...
  t('document parsing failed. Please try again later.')
  t(
...
function _otherI18nKeys(t: (key: string) => string) {
  ...
  t(
    'Only supports basic text files (.txt, .md, .json, code files, etc.). For PDF and Office files, please switch to .'
  )
  t(
  ...
  t(
    'Tries local parsing first without consuming compute points. If local parsing fails, cloud parsing will be used and compute points will be consumed.'
  )
  ...
  t('<OpenSettingButton>Sign in to </OpenSettingButton> to use your account license.')
