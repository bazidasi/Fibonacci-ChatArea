...
export default class DeepSeek extends AbstractAISDKModel {
  ...
  protected getCallSettings(options: CallChatCompletionOptions): CallSettings {
    ...
    settings.providerOptions = {
      deepseek: {
        ...(thinkingType ? { thinking: { type: thinkingType } } : {}),
        // 'xhigh' is only accepted by the DeepSeek-Anthropic transport; the
        // native chat API (and @ai-sdk/deepseek's DeepSeekChatOptions) tops out
        // at 'max', so clamp it at this request boundary.
        ...(reasoningEffort ? { reasoningEffort: reasoningEffort === 'xhigh' ? 'max' : reasoningEffort } : {}),
