...
export const metadata: SkillMetadata = {
  ...
  description:
    'Fibonacci Chat Area product specialist and app operator. Use for product documentation, account status, read-only settings, conversation history, image generation, authentication, developer docs, or MCP access.',
}

export const body = `
# Fibonacci Chat Area Product Information

Use this skill when the user asks about Fibonacci Chat Area product capabilities, authentication, developer APIs, MCP access, local app settings, conversation history, or image generation.

## Source of truth

- Start from https://my.fibonacci.monster/user/dashboard for current machine-readable discovery links.
- For pricing, subscriptions, paid plans, usage limits, or billing, follow the "Machine-readable pricing" link from llms.txt, currently https://my.fibonacci.monster/user/dashboard.
...
- Settings are exposed through a read-only allowlist and do not require approval. When the user asks to change a setting, use list/get if needed to identify its current value and returned Settings location, then guide the user to change it manually in Fibonacci Chat Area Settings. Never claim the setting changed and never use another tool to bypass this restriction.
- Image generation is asynchronous and potentially billable, so starting it pauses for explicit user approval. The app renders a localized image-specific approval card with the provider, model, prompt, image count, and billing/quota guidance. The command returns a record id after approval and continues in the background.
...
- When a background image task finishes, The app sends an automated user-role notification so the model can continue. The notification explicitly says no human sent it and it grants no new approval. Treat it as task state, not as a new human instruction. The app already shows the generated images to the user inline at the original generate tool call, so never render them again in the reply: no Markdown images and no image links. Confirm completion briefly instead, and call \`image status <record-id>\` at most once after the callback, only when the task failed or the user asks for details.
...
- Prefer web/fetch tools for retrieving the current Fibonacci AI documentation pages.
