...
// 这行代码是解决 Windows 通知的标题和图标不正确的问题，标题会错误显示成 electron.app.Fibonacci Chat Area
...
// 开发环境使用 fibonacci-dev:// 协议，避免和正式版冲突
const PROTOCOL_SCHEME = process.defaultApp ? 'fibonacci-dev' : 'fibonacci'
...
function createTray() {
  ...
  tray.setToolTip('Fibonacci Chat Area')
...
async function createWindow() {
  ...
  const qaWindowTitle = MAIN_RUNTIME_POLICY.qaTaskId ? `[QA:${MAIN_RUNTIME_POLICY.qaTaskId}] Fibonacci Chat Area` : undefined
...
if (quitForInstallRequested) {
  ...
  app.on('second-instance', async (_event, commandLine, _workingDirectory) => {
    ...
    const url = commandLine.find((arg) => arg.startsWith('fibonacci://') || arg.startsWith('fibonacci-dev://'))
  ...
  app
    ...
    .then(async () => {
      ...
      if (process.platform !== 'darwin') {
        const url = process.argv.find((arg) => arg.startsWith('fibonacci://') || arg.startsWith('fibonacci-dev://'))
        if (url && mainWindow) {
          // 确保窗口加载完成后再处理 Deep Link
          if (mainWindow.webContents.isLoading()) {
            mainWindow.webContents.once('did-finish-load', () => {
